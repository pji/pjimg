...
class UnitNoise(Noise):
    ...
    def __init__(
        ...
        self.table: Sequence[int] = table
        if self.device and not isinstance(table, torch.Tensor):
            self.table_tensor = torch.tensor(table, device=self.device)
    ...
    # Public methods.
    def fill_array(self, size: Size, location: Loc = (0, 0, 0)) -> ImgAry:
    ...
    def fill_tensor(self, size: Size, loc: Loc = (0, 0, 0)) -> torch.Tensor:
        ...
        t = self._interp_tensor(grids, parts)
        t = t / self.max
        return t
    ...
    def _calc_unit_grid_shape_tensor(self, size: Size):
        """Determine the shape of the unit grid."""
        shape = []
        for axis in range(self._axes):
            # Double inverse floor is ceiling division.
    ...
    def _interp_tensor(
        ...
        if len(grids) > 2:
            ...
            return self._interp_tensor(new_grids, parts)
