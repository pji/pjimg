...
class PerlinTorch(un.UnitNoiseTorch):
    """A class to generate Perlin noise.
    ...
    def fill_tensor(self, size: Size, loc: Loc = (0, 0, 0)) -> torch.Tensor:
        ...
        for grid in grids:
            grids[grid] = self._grad(grid, grids[grid], parts)
        fades = [self._fade(mesh) for mesh in parts]
        t = self._interp(grids, fades)
        t = (t + 1) / 2
        return t
    ...
    def _build_grids(
        ...
        for key in self._hashes:
            ...
            for axis in (Y, X):
                ...
                t_grid = torch.take(self._table, t_grid) + mesh
...
OctavePerlinTorch = un.octave_noise_torch_factory(PerlinTorch, defaults)
BorktavePerlinTorch = un.octave_noise_torch_factory(
    PerlinTorch,
    defaults,
    True
)
