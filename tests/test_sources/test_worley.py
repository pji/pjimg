...
class TestWorley:
    ...
    def test_init_all_optional(self, noise_attr_set):
    ...
    def test_torch_fill(self):
        """Given origin, dimensions, and a color, :meth:`Worley.fill`
        should return a volume filled with a box of the origin,
        dimensions, and color given when the object was created.
        """
        obj = w.Worley(points=6, volume=None, seed='spam', device='cpu')
        result = obj.fill((3, 12, 8))
        assert (mkhex(result) == np.array([
            [
                [0x39, 0x28, 0x39, 0x5a, 0x7f, 0xa6, 0xcd, 0xf5],
...
class TestWorleyCell:
    ...
    def test_fill(self):
        ...
        assert (mkhex(result) == np.array([
            [
                ...
                [0xff, 0xbf, 0xbf, 0xbf, 0xbf, 0xbf, 0x7f, 0x7f],
                [0x3f, 0x3f, 0x3f, 0xbf, 0xbf, 0x7f, 0x7f, 0x7f],
            ...
            [
                [0xbf, 0xbf, 0xbf, 0xbf, 0xbf, 0xbf, 0xbf, 0xbf],
                [0xbf, 0xbf, 0xbf, 0xbf, 0xbf, 0xbf, 0xbf, 0x7f],
                [0x3f, 0x3f, 0xbf, 0xbf, 0xbf, 0x7f, 0x7f, 0x7f],
                [0x3f, 0x3f, 0x3f, 0x3f, 0x7f, 0x7f, 0x7f, 0x7f],
                [0x3f, 0x3f, 0x3f, 0x7f, 0x7f, 0x7f, 0x7f, 0x7f],
                [0x3f, 0x3f, 0x3f, 0x7f, 0x7f, 0x7f, 0x7f, 0x7f],
                [0x3f, 0x3f, 0x7f, 0x7f, 0x7f, 0x7f, 0x7f, 0x7f],
            ...
            [
                [0xbf, 0xbf, 0xbf, 0xbf, 0xbf, 0xbf, 0xbf, 0x7f],
                [0x3f, 0xbf, 0xbf, 0xbf, 0xbf, 0xbf, 0x7f, 0x7f],
                [0x3f, 0x3f, 0x3f, 0xbf, 0xbf, 0x7f, 0x7f, 0x7f],
                [0x3f, 0x3f, 0x3f, 0x3f, 0x7f, 0x7f, 0x7f, 0x7f],
                [0x3f, 0x3f, 0x3f, 0x7f, 0x7f, 0x7f, 0x7f, 0x7f],
                [0x3f, 0x3f, 0x3f, 0x7f, 0x7f, 0x7f, 0x7f, 0x7f],
    ...
    def test_fill_antialias(self):
        """Given origin, dimensions, and a color, :meth:`Worley.fill`
        should return a volume filled with a box of the origin,
        dimensions, and color given when the object was created. If
        antialias on the object is `True`, then the edges of the
        cells should be softened.
        """
        obj = w.WorleyCell(points=5, volume=None, seed='spam', antialias=True)
        result = obj.fill((3, 12, 8))
        assert (mkhex(result) == np.array([
            [
    ...
    def test_torch_fill(self):
        ...
        dimensions, and color given when the object was created.
        """
        obj = w.WorleyCell(points=5, volume=None, seed='spam', device='cpu')
        result = obj.fill((3, 12, 8))
        assert (mkhex(result) == np.array([
            [
                [0xbf, 0xbf, 0xbf, 0xbf, 0xbf, 0xbf, 0xbf, 0xbf],
...
class TestOctaveWorleyCell:
    # Tests for Worley initialization.
    def test_init_all_default(self):
        """Given only required parameters, :class:`OctaveWorleyCell`
        should initialize the required attributes with the given values.
        It should then initialize the optional attributes with default
        values.
        """
        required = {}
        optional = {
        ...
        obj = w.OctaveWorleyCell(**required)
        for attr in required:
            assert getattr(obj, attr) == required[attr]
        for attr in optional:
            assert getattr(obj, attr) == optional[attr]
    ...
    def test_init_all_optional(self):
        """Given optional parameters, :class:`OctaveWorleyCell` should
        initialize the given attributes with the given values.
        """
        required = {}
        optional = {
        ...
        obj = w.OctaveWorleyCell(**required, **optional)
        for attr in required:
            assert getattr(obj, attr) == required[attr]
        for attr in optional:
            assert getattr(obj, attr) == optional[attr]
    ...
    def test_fill(self):
        ...
        assert (mkhex(result) == np.array([
            [
                [0x59, 0x90, 0x95, 0xaf, 0xb0, 0xb0, 0x82, 0x77],
                [0x2c, 0x3b, 0x3b, 0xa5, 0xd0, 0x8b, 0x7d, 0x77],
                [0x34, 0x16, 0x7e, 0xa3, 0x9d, 0x91, 0xa3, 0x5c],
                [0x75, 0x99, 0xbb, 0xa3, 0x8c, 0x6b, 0x9d, 0x5c],
                [0x81, 0x7a, 0xb3, 0x7e, 0x5d, 0x16, 0x46, 0x77],
                [0x89, 0x69, 0x6b, 0xb4, 0x84, 0x66, 0x70, 0x9b],
                [0x9a, 0x6e, 0x8a, 0xab, 0x5c, 0x31, 0x6d, 0x83],
                [0x97, 0x80, 0xb0, 0x9a, 0x61, 0x6a, 0x65, 0x61],
            ],
        ], dtype=np.uint8)).all()
