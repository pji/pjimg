...
# Defaults.
OctaveNoiseDefaults = sources.unitnoise.OctaveNoiseDefaults
...
def make_noise(args: ap.Namespace) -> ImgAry:
    ...
    :rtype: numpy.ndarray
    """
    noise = sources.Noise(seed=args.seed)
    size = (1, args.height, args.width)
    loc = (0, *args.location)
    return noise.fill(size, loc)
...
def make_ocoscurtains(args: ap.Namespace) -> ImgAry:
    ...
    :rtype: numpy.ndarray
    """
    noise = sources.OctaveCosineCurtains(
...
def make_spheres(args: ap.Namespace) -> ImgAry:
    ...
    :rtype: numpy.ndarray
    """
    noise = sources.Spheres(radius=args.radius, offset=args.offset)
    size = (1, args.height, args.width)
    loc = (0, *args.location)
    return noise.fill(size, loc)
...
def parse_operlin(spa: ap._SubParsersAction) -> None:
    ...
    add_octave_arguments(sp, sources.perlin.defaults)
    add_unitnoise_arguments(sp)
    add_noise_arguments(sp)
...
def add_maze_arguments(sp: ap.ArgumentParser) -> None:
    ...
    noise arguments too, since :class:`sources.Maze` needs a different
    default value for :attr:`sources.UnitNoise.unit`.
